    ...
    : BMediaAddOn(imid)
{
    ...
    fMediaNodeFlavorInfo.name = "DVD Media";
    fMediaNodeFlavorInfo.info = "Extracts the DVD media and navigation data.";
        ...
        status_t *out_error)
{
    ...
    if (info->internal_id == fMediaNodeFlavorInfo.internal_id) {
        DVDMediaNode *node = new DVDMediaNode(this, fMediaNodeFlavorInfo.name, fMediaNodeFlavorInfo.internal_id);
        ...
        DVDExtractorNode *node = new DVDExtractorNode(&fExtractorNodeFlavorInfo, this);
        if (node && (node->InitCheck() < B_OK)) {
            delete node;
            return NULL;
        } else {
...
{
    return new DVDMediaAddOn(image);
}
